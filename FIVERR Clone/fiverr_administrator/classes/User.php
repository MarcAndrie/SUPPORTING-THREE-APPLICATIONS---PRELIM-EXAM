...
class User extends Database {
     ...
     * @param bool $is_fiverr_administrator Whether the user is a fiverr administrator.
     * @return bool True on success, false on failure.
     */
    public function registerUser($username, $email, $password, $contact_number, $is_fiverr_administrator = 0) {
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        $is_client = $is_fiverr_administrator ? 1 : 0; // Fiverr administrators are also clients
        $sql = "INSERT INTO fiverr_clone_users (username, email, password, is_client, contact_number, is_fiverr_administrator) VALUES (?, ?, ?, ?, ?, ?)";
        try {
            $this->executeNonQuery($sql, [$username, $email, $hashed_password, $is_client, $contact_number, $is_fiverr_administrator]);
            return true;
    ...
    public function loginUser($email, $password) {
        $sql = "SELECT user_id, username, password, is_client, is_fiverr_administrator FROM fiverr_clone_users WHERE email = ?";
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            $_SESSION['is_fiverr_administrator'] = (bool)$user['is_fiverr_administrator'];
            return true;
        }
        return false;
    }
    ...
    public function isFiverrAdministrator() {
        $this->startSession();
        return isset($_SESSION['is_fiverr_administrator']) && $_SESSION['is_fiverr_administrator'];
    }
